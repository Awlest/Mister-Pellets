...
| `MOLLIE_API_KEY` | `live_xxxxxxxxxxx` | Mollie Dashboard (mode normal, pas test) → Developers → API keys |
...
## Setup Mollie Live (10 min)

Mollie remplace Stripe (V1.6) pour profiter du **forfait Bancontact à 0,39 €**
(vs 1,4 % + 0,25 € chez Stripe). Sur un poêle à 5 000 €, économie de ~70 €
par transaction Bancontact. Pas d'abonnement mensuel, commission seule.

1. **Créer un compte sur https://www.mollie.com**
   - Email pro (info@awlest.com)
   - Plan gratuit (commission seule)
2. **Activation du compte** (5-10 min) :
   - Mollie demande un dossier d'inscription : nom Awlest SRL, TVA BE 0656.514.212,
     IBAN du compte qui recevra les paiements, justificatifs (registre commerce, ID gérant)
   - Validation par Mollie sous 1-2 jours ouvrés
3. **Activer Bancontact** dans Dashboard → Settings → Payment methods
   - Cocher : Bancontact (forfait 0,39 €), Cartes de crédit (Visa/Mastercard 1,8 %), Apple Pay (gratuit)
4. **Récupérer la clé API** :
   - Dashboard → Developers → API keys
   - **Live API key** (commence par `live_`) → copier dans `MOLLIE_API_KEY` Vercel env vars
   - Pas besoin de webhook secret (Mollie ne signe pas les webhooks, sécurité par
     ID payment non-devinable)
5. **Brander la page de paiement** :
   - Dashboard → Settings → Branding : logo Mister Pellets + couleurs

**Pas besoin de configurer le webhook côté Mollie Dashboard** : notre code
passe automatiquement l'URL `webhookUrl` à chaque création de payment, donc
Mollie sait où POST quand le paiement aboutit.
...
## Migration base de données prod

Une fois la base prod créée et `DATABASE_URI` configuré sur Vercel :

```bash
# Depuis ton poste local, après avoir mis DATABASE_URI prod dans .env.local :
pnpm payload migrate
# Confirmer "y" si demande (data loss = OK car base prod vide)
```

Ou plus propre, exécuter via Vercel CLI :

```bash
vercel env pull .env.production.local --environment=production
DOTENV_CONFIG_PATH=.env.production.local pnpm payload migrate
```

Migrations qui seront appliquées :
- 20260502_074301 : schéma initial (Users, Media, Products, Orders, Quotes, ContactMessages)
- 20260502_102047, 20260502_103121, 20260502_112806 : ajustements Phase 5
- 20260504_080000_mollie_migration : remplace stripe_payment_intent_id +
  stripe_checkout_session_id par mollie_payment_id sur la table orders (V1.6)

Après migration, créer le premier admin via `https://mister-pellets.be/admin`
(formulaire d'inscription affiché tant qu'aucun user n'existe).
...
- [ ] Checkout test : sélectionner Bancontact dans Mollie (test avec une vraie CB en très petit montant : ~1 €). Vérifier redirection vers `/commande/{orderId}` après paiement
- [ ] Webhook Mollie reçu (vérifier dans Mollie Dashboard → Payments → cliquer sur le payment → Activity → "Webhook called")
- [ ] Commande passée à statut "paid" dans Payload Admin (mister-pellets.be/admin → Orders)
